...
func (d *Database) GetPhoneNumber(userID string) (string, error) {
	...
	err := d.DB.Get(&phoneNumber, "SELECT phone_number FROM profile WHERE user_id = ?", userID)
...
func (d *Database) IsPhoneNumberDuplicate(phoneNumber string) (bool, error) {
	...
	err := d.DB.Get(&existingID, "SELECT user_id FROM profile WHERE phone_number = ? LIMIT 1", phoneNumber)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
...
func (d *Database) UpdatePhoneNumber(userID string, phoneNumber string) error {
	_, err := d.DB.Exec("UPDATE profile SET phone_number = ? WHERE user_id = ?", phoneNumber, userID)
	return err
}
