...
export default function TxHeader({ data, phase }: { data: FleaThreadResponse; phase: TxPhase }) {
  ...
  return (
    <div style={s.wrap}>
      <div style={s.img}>
        {item?.main_image_url && <img src={CONFIG.BASE_URL + item.main_image_url} style={{ width: "100%", height: "100%", objectFit: "cover" }} />}
      </div>
      <div style={s.info}>
        ...
        <div style={{ marginTop: 4, fontSize: 12, color: "#5c5a56" }}>{sub}:</div>
...
function StatusBadge({ phase }: { phase: TxPhase }) {
  const label: Record<TxPhase, string> = {
    SELLER_SET_TERMS: "条件確定（出品者）", BUYER_WAIT_TERMS: "条件待ち（購入者）",
    BUYER_CONFIRM: "購入者の同意待ち", PAYMENT: "決済", SHIPPING: "発送/受取",
    COMPLETE: "完了", CANCELLED: "キャンセル", UNKNOWN: "不明",
  };
