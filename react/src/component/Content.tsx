...
type Props = {
  item: Content & { is_selected?: boolean };
  function: (item: Content) => void;
  is_selected?: boolean;
};
...
const CartContent: React.FC<Props> = ({
  ...
  const ChangeCheck = (item: CartItem, is_selected: boolean) => {
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between space-y-2 text-gray-800">
        ...
        <p className="text-left">
          価格：<span className="font-medium">{item.price}円</span>
        </p>
        <p className="text-left">
          ポイント：<span className="font-medium">{item.point}pt</span>
        </p>
        <p className="text-left">
          数量：
          <input
