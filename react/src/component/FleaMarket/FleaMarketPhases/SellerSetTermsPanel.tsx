...
export default function SellerSetTermsPanel({
  ...
  const [shippingFeeType, setShippingFeeType] = useState(
    transaction?.shipping_fee_type,
  );
  const [priceShipping, setPriceShipping] = useState<number>(
    transaction?.price_shipping ?? 0,
  );
  ...
  async function submit() {
    ...
    try {
      ...
      await api.post(
        `/flea/transactions/${transaction?.id}/seller/confirm-terms`,
        {
    ...
    <div className="rounded-2xl border bg-white p-4 shadow-sm space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <select
          ...
          onChange={(e) =>
            setShippingFeeType(e.target.value as ShippingFeePref)
          }
          disabled={busy}
        >
