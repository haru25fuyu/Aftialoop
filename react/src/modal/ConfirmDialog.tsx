  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
    ...
    <span
      style={{
        ...
        fontWeight: isMoney ? 700 : 500,
        color: isMoney ? "#1a5adc" : "#1a1a1a",
      }}
    >
      {value}
    </span>
...
export function ConfirmDialog({
  ...
  const displayUrls = useMemo(
    () =>
      summary.images.map((img) =>
        img.file
          ? { src: URL.createObjectURL(img.file), isBlob: true }
          : { src: img.url, isBlob: false },
      ),
    [summary.images],
  );
  useEffect(
    () => () => {
      displayUrls.forEach((d) => {
        if (d.isBlob) URL.revokeObjectURL(d.src);
      });
    },
    [displayUrls],
  );
  ...
  const shipFeeLabel =
    shippingFeeType === 0 ? "送料込み (出品者負担)" : "着払い (購入者負担)";
  const shipsLabel = !shipsWithinDays
    ? "未選択"
    : shipsWithinDays === 1
      ? "1日以内"
      : shipsWithinDays === 2
        ...
        : shipsWithinDays === 4
          ...
          : `${shipsWithinDays}日以内`;
  const prefName =
    PREFS?.find((p: { id: number; name: string }) => p.id === shipFromId)?.name || "未設定";
        ...
        <div style={s.body}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            >
                ...
                value={
                  <span
                    style={{
                      ...
                      padding: "2px 8px",
                      borderRadius: 9999,
                      backgroundColor: isSupply ? "#e8f0fe" : "#f0fae8",
                      color: isSupply ? "#1a5adc" : "#3a7a22",
                      fontWeight: 700,
                    }}
                  >
                    {typeLabel}
              ...
              <Row
                label="数量"
                value={`${quantity} 個 ${isMultiPurchasable ? "(複数可)" : ""}`}
              />
              {sellerPlusPct > 0 && (
                <Row label="追加割引" value={`${sellerPlusPct}%`} />
              )}
