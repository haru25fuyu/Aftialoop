...
export default function PurchaseRequestModal({
  ...
  const [shippingMethodPref, setShippingMethodPref] = useState<ShippingMethod>(
    ShippingMethod.SELLER_CHOICE,
  );
  const [shippingFeePref, setShippingFeePref] = useState<ShippingFeePref>(
    ShippingFeePref.OK_EITHER,
  );
  ...
  useEffect(() => {
    ...
    api
      .post("/address/list")
      .then((res) => {
        const list: Address[] = res.data.address || [];
        ...
        <div style={s.card}>
          ...
          <div style={s.body}>
            ...
            <div style={s.section}>
              ...
              {address ? (
                <div
                  style={{ fontSize: 14, color: "#2e3128", lineHeight: 1.7 }}
                >
                ...
                {SHIP_METHODS.map(({ value, label }) => (
                  <label
                    ...
                    style={{
                      ...
                      padding: "12px 16px",
                      borderRadius: 8,
                      border: `2px solid ${shippingMethodPref === value ? "#1a1a1a" : "#e0ddd8"}`,
                      cursor: "pointer",
                      backgroundColor:
                        shippingMethodPref === value ? "#f8f7f5" : "#fff",
                    }}
                  >
                    <input
                      type="radio"
                      checked={shippingMethodPref === value}
                      onChange={() => setShippingMethodPref(value)}
                      style={{ width: 18, height: 18 }}
                    />
                    <span
                      style={{
                        fontSize: 14,
                        fontWeight: shippingMethodPref === value ? 700 : 400,
                      }}
                    >
                      {label}
                    </span>
                ...
                {FEE_PREFS.map(({ value, label }) => (
                  <label
                    ...
                    style={{
                      ...
                      padding: "12px 16px",
                      borderRadius: 8,
                      border: `2px solid ${shippingFeePref === value ? "#1a1a1a" : "#e0ddd8"}`,
                      cursor: "pointer",
                      backgroundColor:
                        shippingFeePref === value ? "#f8f7f5" : "#fff",
                    }}
                  >
                    <input
                      type="radio"
                      checked={shippingFeePref === value}
                      onChange={() => setShippingFeePref(value)}
                      style={{ width: 18, height: 18 }}
                    />
                    <span
                      style={{
                        fontSize: 14,
                        fontWeight: shippingFeePref === value ? 700 : 400,
                      }}
                    >
                      {label}
                    </span>
            ...
            <div style={s.section}>
              ...
              <textarea
                ...
                placeholder="取引に関するご要望などがあればどうぞ"
                rows={3}
                style={{
                  width: "100%",
                  padding: 12,
