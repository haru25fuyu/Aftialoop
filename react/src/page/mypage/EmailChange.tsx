        ...
        return (
            <>
                ...
                <div className="min-h-screen bg-white p-6 flex flex-col items-center justify-center text-center">
                    ...
                    <button onClick={() => navigate("/mypage")} className="bg-blue-600 text-white font-bold py-3 px-10 rounded-full">
                        マイページへ戻る
                    </button>
                </div>
            </>
                ...
                <main className="max-w-lg mx-auto p-4 space-y-6">
                    ...
                    <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
                        {step === "INPUT" && (
                            <>
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">新しいメールアドレス</label>
                                    <input
                                        type="email"
                                ...
                                >
                                    {loading ? <Loader2 className="animate-spin" /> : <><Send size={18} /> 認証コードを送る</>}
                                </button>
                            </>
                        )}
                        ...
                        {step === "VERIFY" && (
                            <>
                                <div className="text-center mb-4">
                                    ...
                                    <p className="text-xs text-gray-400">届かない場合は迷惑メールフォルダを確認してください</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2 text-center">認証コード</label>
                                    <input
                                        type="text"
                                ...
                                >
                                    {loading ? <Loader2 className="animate-spin" /> : "変更を確定する"}
                                </button>
                            </>
                        )}
                    </section>
