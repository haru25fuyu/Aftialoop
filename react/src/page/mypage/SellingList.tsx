...
const statusBadge: Record<number, { label: string; color: string }> = {
  0: { label: "下書き", color: "#7a3fa0" },
  1: { label: "出品中", color: "#1a5adc" },
  2: { label: "取引中", color: "#b85c00" },
  3: { label: "売却済", color: "#5c5a56" },
  4: { label: "出品取り消し", color: "#d63c20" },
};
    ...
    const fetch = async () => {
      ...
      try {
        const res = await api.get(
          `/flea-market/selling/list?limit=${limit}&offset=${offset}`,
        );
        ...
        setItems((prev) => {
          ...
          return [
            ...prev,
            ...newItems.filter((i: ListingItem) => !ids.has(i.id)),
          ];
        });
