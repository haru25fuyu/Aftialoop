...
const UserProfile: React.FC = () => {
  ...
  const lastListingRef = useCallback(
    (node: HTMLAnchorElement | null) => {
      ...
      const obs = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) fetchListings(listings.length, false);
      });
  ...
  const lastReviewRef = useCallback(
    (node: HTMLDivElement | null) => {
      ...
      const obs = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) fetchReviews(reviews.length, false);
      });
      if (node) obs.observe(node);
    },
  ...
  const fetchListings = async (offset: number, reset: boolean) => {
    ...
    try {
      const res = await api.get(
        `/user/profile/${profile.id}/listings?limit=${LIMIT}&offset=${offset}&status=${FleaItemStatus.Active}`,
      );
  ...
  const fetchReviews = async (offset: number, reset: boolean) => {
    ...
    try {
      const res = await api.get(
        `/user/profile/${profile.id}/reviews?limit=${LIMIT}&offset=${offset}`,
      );
  ...
  useEffect(() => {
    if (profile && activeTab === "listings") fetchListings(0, true);
  }, [profile, activeTab]);
  useEffect(() => {
    if (profile && activeTab === "reviews") fetchReviews(0, true);
  }, [profile, activeTab]);
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      if (prev) await api.delete(`/sns/users/${profile.id}/follow`);
  ...
  const handleBlock = async () => {
    ...
    if (window.confirm("ブロックしますか？")) {
      try {
        await api.post(`/sns/users/${profile!.id}/block`, {});
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "center" }}>
              ...
              {profile.username && (
                <p style={{ color: "#8c8c8c", marginTop: 2 }}>
                  @{profile.username}
                </p>
              )}
            </div>
            <div style={s.statsRow}>
              {[
                ...
                <div key={label as string} style={s.statItem}>
            ...
            {!isMe && (
              <div style={s.actionRow}>
                <button
                  ...
                  style={s.followBtn(profile.isFollowing)}
                >
                  {profile.isFollowing ? "フォロー中" : "フォローする"}
                </button>
                <button onClick={handleBlock} style={s.blockBtn}>
                  ブロック
                </button>
              </div>
            )}
        ...
        {activeTab === "reviews" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ color: "#f0a800", fontSize: 12 }}>
                ...
                <p style={{ fontSize: 14, color: "#5c5a56", lineHeight: 1.6 }}>
            ...
            {!reviewLoading && reviews.length === 0 && (
              <div
                style={{ textAlign: "center", padding: 80, color: "#8c8c8c" }}
              >
                まだ評価はありません
              </div>
            )}
