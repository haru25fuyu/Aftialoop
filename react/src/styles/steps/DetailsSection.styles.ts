...
input: { ...inputBase },
select: { ...inputBase },
sexGrid: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: spacing[2] },
sexBtn: (active: boolean) => ({ cursor: "pointer", border: `1px solid ${active ? colors.neutral900 : semantic.borderDefault}`, borderRadius: radius.lg, padding: spacing[3], textAlign: "center" as const, backgroundColor: active ? colors.neutral100 : "transparent", fontWeight: active ? fontWeight.bold : fontWeight.normal, fontSize: fontSize.sm }),
sizeRow: { display: "flex", gap: spacing[2] },
sizeInput: { ...inputBase, flex: 1 },
sizeUnitSelect: { ...inputBase, width: 80 },
supplyGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: spacing[4] },
};
